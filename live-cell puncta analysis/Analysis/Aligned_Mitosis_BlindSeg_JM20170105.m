...
punctaMatrix = NaN(length(incdatatotal)+length(lowdatatotal)+length(emergdatatotal),3);
inczero = find(alignedtime1 == 0);
for i = 1:size(incdatatotal,1)
    if sum(isnan(incpuncta_datatotal(i,inczero+interval))) < 1
...
for i = 1:size(lowdatatotal,1)
    if sum(isnan(lowpuncta_datatotal(i,lowzero+interval))) < 1
        punctaMatrix(x+i,1) = nansum(lowpuncta_datatotal(i,lowzero+interval));
        punctaMatrix(x+i,2) = nanmean(lowpuncta_datatotal(i,lowzero+interval));
        punctaMatrix(x+i,3) = nanmedian(lowpuncta_datatotal(i,lowzero+interval));
    else
        punctaMatrix(x+i,1:3) = NaN;
    end
end
...
for i = 1:size(emergdatatotal,1)
    if sum(isnan(emergpuncta_datatotal(i,emergzero+interval))) < 1 
        punctaMatrix(x+i,1) = nansum(emergpuncta_datatotal(i,emergzero+interval));
        punctaMatrix(x+i,2) = nanmean(emergpuncta_datatotal(i,emergzero+interval));
    else
        punctaMatrix(x+i,1:3) = NaN;
    end
end
...
topSum = prctile(critSumval,95);
bottomSum = prctile(critSumval,5);
...
for i = 1:(length(tops) + length(bottoms))
    ...
    if i <= length(tops)
        index = ixsum(tops(i));
        plot = 0;
    elseif i <= length(bottoms)
        index = ixsum(bottoms(i));
        plot = 2;
    end
...
topMean = prctile(critMeanval,95);
bottomMean = prctile(critMeanval,5);


tops = find(critSumval >= topMean);
bottoms = find(critSumval <= bottomMean);
figure 
...
topMed = prctile(critMedval,95);
bottomMed = prctile(critMedval,5);


tops = find(critSumval >= topMed);
bottoms = find(critSumval <= bottomMed);
figure 
